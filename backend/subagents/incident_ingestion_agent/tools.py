...
INDEX_ENDPOINT_ID = os.getenv("INDEX_ENDPOINT_ID","projects/vodaf-aida25lcpm-206/locations/europe-west1/indexEndpoints/5059503910281019392")
DEPLOYED_INDEX_ID = os.getenv("DEPLOYED_INDEX_ID","Test_INDEX_ENDPOINT_20250910101151")
INDEX_NAME = os.getenv("INDEX_NAME","projects/vodaf-aida25lcpm-206/locations/europe-west1/indexes/7364009913355337728")
...
def ingest_incident_data(incident_data: Union[Dict[str, Any], List[Dict[str, Any]]]) -> str:
    """
    Ingest incident record(s) into the vector database.
    
    Args:
        incident_data: Dictionary containing incident information OR List of incident dictionaries
    ...
    if isinstance(incident_data, list):
        # If it's a list, use batch processing
        return batch_ingest_incidents(incident_data)
    elif isinstance(incident_data, dict):
        # If it's a single incident, process as before
        pass
    else:
        return "❌ Error: incident_data must be a dictionary or list of dictionaries"
    ...
    try:
        ...
        # Step 8: Upload to GCS bucket (backup method)
        gcs_success = False
        try:
